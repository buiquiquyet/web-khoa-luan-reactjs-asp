...
function CommentForm({idForum, typePost}) {
    ...
    const handleSubmit = async () => {
        if(localStorage.getItem('name')) {
            let obj = {}
            if( commentTextArea === "") {
                NotificationManager.warning("Nhập đầy đủ các thông tin", 'Cảnh báo', 1000);
                return    
            }
            ...
            if(rs) {
                ...
                NotificationManager.success("Gửi bình luận thành công", 'Thành công', 1000);
            }else {
    ...
    const handleSubmitEditComment = async () => {
        ...
        const rs = await fecthCommentApiEdit(dataEditComment.CommentId,dataEditComment)
        if(rs) {
            ...
            <div ref={modalRef} className={cx('modal')}>
                ...
                <div className={cx('modal-content')}>
                   <div className={cx("comment-input")}>
                        ...
                        ></div>
                        <button onClick={handleSubmitEditComment} className={cx("comment-button")}>
                    ...
                    {!checkCommentUser
                        ...
                        <div className={cx("comment-input")}>
                            ...
                            <div className={cx('comment-box')}   
                                ...
                                onBlur={handleChangeDiv}
                            >  
                            
                            </div>
                            <button onClick={handleSubmit} className={cx("comment-button")} ><SendIcon classsName={cx('commment-iconSend')}/></button>
                        </div>  
            ...
            
            {
                ...
                <div className={cx('comment-show')}>
                    ...
                    <div className={cx('comment-loadMore')}>
                        ...
                        <span ref={hideCommentRef} onClick={handleHideCommemt} className={cx('hide-less')}>Ẩn bớt</span>
                    </div>}
                </div>
            }
        </div>
